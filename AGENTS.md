...
A personal golf journey blog. Content-driven, statically rendered where possible. No database or auth. The primary workflow is: author writes Markdown → deploys → readers browse posts. Built with TanStack Start and deployed on Netlify.

## Golf Journal–Specific Notes

- **Stats on the homepage hero** (handicap, best round, rounds played, goal) are hardcoded in `src/routes/index.tsx` — update them manually as the author's stats change.
- **Post images** live in `/public/` and are referenced by filename in frontmatter. `placeholder.png` is the safe fallback; the `onError` handler in post cards falls back to a CSS gradient automatically.
- **Categories used**: `Course Diary`, `Milestones`, `Lessons Learned`, `Competition` — stick to these for consistent filtering.
- **Design system**: All layout, typography, and color is in `src/styles.css` using CSS custom properties. Do not use Tailwind utility classes for color or typography — use the named CSS classes (`.post-card-title`, `.hero-title`, etc.) and variables (`--green-deep`, `--gold`, `--cream`).
- **Slug generation**: Auto-derived from title in `content-collections.ts` transform — hyphens, lowercase, special chars stripped.
